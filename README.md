...
Final educational project that brings together previous learnings into a complete file management API. It demonstrates practical mastery of authentication, Express/Node.js, MongoDB, Redis, pagination, and background image processing with thumbnails.
...
| **FILES**         | **DESCRIPTION**                                                                                 |
| :---------------: | ----------------------------------------------------------------------------------------------- |
| `controllers`     | API logic: `AppController.js`, `UsersController.js`, `AuthController.js`, `FilesController.js`. |
| `routes`          | API routing (`index.js`).                                                                       |
| `utils`           | Helpers: `db.mjs` (MongoDB), `redis.mjs` (Redis).                                               |
| `server.js`       | Express server setup and bootstrap.                                                             |
| `worker.js`       | Bull worker for image thumbnails (100/250/500 px).                                              |
...
```bash
git clone https://github.com/fchavonet/holbertonschool-files_manager.git
```
...
4. Ensure MongoDB and Redis are running locally, and create the storage folder if you customize it:
...
```bash
# 1) Create a user.
curl -s -X POST http://0.0.0.0:5000/users \
  -H "Content-Type: application/json" \
  -d '{ "email": "batman@batcave.com", "password": "iloverobin" }'

# 2) Connect (Basic Auth). It's just "email:password" in base64.
TOKEN=$(curl -s http://0.0.0.0:5000/connect \
  -H "Authorization: Basic YmF0bWFuQGJhdGNhdmUuY29tOmlsb3Zlcm9iaW4=" \
  | jq -r .token)

echo "Token received: $TOKEN"

# 3) Upload a text file (base64 of: 'Hello Webstack!\n').
curl -s -X POST http://0.0.0.0:5000/files \
  -H "X-Token: $TOKEN" -H "Content-Type: application/json" \
  -d '{ "name": "myText.txt", "type": "file", "data": "SGVsbG8gV2Vic3RhY2shCg==" }'

# 4) List files.
curl -s -X GET http://0.0.0.0:5000/files -H "X-Token: $TOKEN" | jq

# 5) Get raw content (replace <fileId> with the id from step 4).
# Alfred probably left you a note in there...
# Example:
# curl -s http://0.0.0.0:5000/files/<fileId>/data
```

> Refer to the <a href="#instructions">Instructions</a> section for more details.
